...
class CalorieEntriesController < ApplicationController
  ...
  def create
    ...
    if @calorie_entry.save
      @calorie_entry.analyze! if run_ai_analysis?
      respond_to do |format|
        format.turbo_stream
        format.html { redirect_to daily_log_path(@date), notice: after_create_notice }
      end
    else
  ...
  def run_ai_analysis?
    truthy?(params[:run_ai_analysis]) && image_param_present?
  end
